...
| 1920 年代定格动画胶片：象牙纸上的铅笔与水彩，每秒 8 个跳动的姿势，标本标签、纸片字卡、唯一一抹酒红，最后一记砸屏大标题。 | 多雷式铜版画：棕褐纸面、交叉排线一层层堆出阴影、笔尖跟着最新一笔走，只有一种暖色灯光。 | 掐丝珐琅：金丝自己盘出轮廓，珐琅釉一格格流满，然后发光。古筝配乐。只固定金丝，配色随你的故事换（预览是月夜粉彩那一版）。 |
| 1920s stop-motion reel: pencil and wash on ivory paper, poses that jump 8 times a second, specimen labels, paper caption cards, one wine red, a slam title. | Doré-style copperplate: sepia paper, cross-hatching that builds the shadows, a pen nib that follows the newest stroke, one warm lamp colour. | Cloisonné: gold wire lays itself, enamel floods each cell, then it glows. Guzheng score. Only the gold is fixed; pick your own enamel palette (the preview is the pastel-night one). |
...
- `styles/*.md`：每种画风一份指南，含色板、工具函数、动作词汇、音效事件，以及「怎么用这个画风做你自己的片子」。
  One guide per style: palette, helpers, motion vocabulary, sound events, and how to build your own film in it.
...
- `examples/goldseam`：掐丝珐琅的 9.5 秒小样《金缮》（一只茶盏自己掐丝点蓝、碎掉、再用金丝补好）。它是一个没见过我们的模型只读指南画出来的。
  A 9.5-second liyue demo (a teacup that makes itself, shatters, and is mended with gold), made by a fresh agent from the style guide alone.
...
做新片：先读 `SKILL.md` 里的「起手卡」，再读你想要的画风指南，从对应的示例抄起。
For a new film, read the starter cards in `SKILL.md`, then your style's guide, and start from its example.
...
由 **Avenil & Oshra** 制作。欢迎贡献新画风：加一个 `engine/<style>/`、一份 `styles/<style>.md`，一个示例，再在 `SKILL.md` 的起手卡里加一列。
Made by **Avenil & Oshra**. New style drawers are welcome: add `engine/<style>/`, `styles/<style>.md` an example, and a column in the starter cards in `SKILL.md`.
